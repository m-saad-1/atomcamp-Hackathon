...
# Part 5 — Enterprise Security Edge Cases, ARIA Specifics & Backend Hardening
...
> - Parts 1 through 4
>
> This section details the advanced, enterprise-grade handling of edge cases, token lifecycles, advanced security protocols, and strict ARIA requirements.

---

# 1. Advanced Token Expiry Flows

## Proactive Token Refresh
- **Token Lifespans**: Access tokens must expire every 15 minutes. Refresh tokens rotate and expire every 7 days (or 30 days if "Remember Me" is checked).
- **Silent Refresh**: The frontend must initiate a silent refresh 60 seconds before the access token expires.
- **Refresh Blacklisting**: Once a refresh token is used to generate a new pair, it is immediately blacklisted. If a blacklisted token is reused (indicative of token theft), the entire token family is instantly revoked, and the user is forcefully logged out of all active sessions.

## Stale Tab Re-authentication
If a user leaves a tab open and the session expires, upon returning and attempting an action (like submitting a form):
1. The request is intercepted.
2. An **inline authentication modal** appears, retaining the state of the underlying form.
3. Upon successful re-authentication, the intercepted request is automatically retried without data loss.

---

# 2. Security Edge Cases & Rate Limiting Strategies

## Credential Stuffing & Brute Force Prevention
- **Progressive Delays**: After 3 failed login attempts from a single IP, introduce a 5-second artificial delay.
- **Account Lockout**: After 10 consecutive failed attempts on a single account, the account is temporarily locked for 30 minutes. The user receives an email notification containing an instant-unlock link.
- **Impossible Travel Detection**: If a login occurs in New York and 10 minutes later another login occurs in Tokyo, the session is flagged. An additional Email OTP is required to proceed.

## Session Hijacking Mitigations
- **Device Fingerprinting**: Sessions are bound to a loose device/browser fingerprint. A sudden change in User-Agent or IP subnet for the same session triggers a re-authentication prompt.

---

# 3. Strict ARIA & Accessibility Mandates

## Focus Management
- When an inline error appears, focus must **not** be stolen unless the error is critical to the entire form (e.g., global submission failure).
- If a form submission fails, focus must programmatically move to the first input field containing an error.

## Dynamic Content & Live Regions
- All progress steppers and spinners must be wrapped in `aria-live="polite"`.
- Critical authentication failures (e.g., "Account Locked") must use `aria-live="assertive" role="alert"`.
- Password strength meters must announce their state to screen readers (e.g., "Password strength: Strong") as the user pauses typing.

## Modal & Dialog Traps
- When a "Session Expired" or "Forced Logout" modal appears, focus must be trapped within the modal (`aria-modal="true"`). The rest of the DOM should be hidden from screen readers using `aria-hidden="true"`.